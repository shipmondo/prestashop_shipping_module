...
jQuery(window).on('load', function() { //jQuery(document).ready(function () {
    ...
    jQuery('.payment_module').on('click', function(event) {
        if (jQuery('#Pakkelabels_zipcode_field').length > 0) {
            if (jQuery('#cgv').is(':checked') && jQuery('.payment_module:visible').length != 0 && jQuery('#selected_shop_context > .pakkelabels-Packetshop:visible').length) {} else {
                event.preventDefault();
            }
        }
    })
